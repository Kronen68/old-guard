...
local function add_stat_text(desc, currentTurret, chargeMax)
	...
	if currentTurret.chain and currentTurret.chain.type == chain_types.cooldown then
		...
		desc = desc.."\nЭффект цепи: снижение времени перезарядки на "..chain_amount.."%"
		...
		desc = desc.."\nМин. перезарядка цепи: "..chain_max_effect.."% (после "..chain_count.."-го залпа)"
	end
	desc = desc.."\n\nСкорость вращения: "..math.floor(currentTurret.rotation_speed)
	if currentTurret.shot_radius then
		desc = desc.."\nРазброс выстрела: "..math.floor(currentTurret.shot_radius)
	end
	desc = desc.."\nСкорость стрельбы: "
	...
	desc = desc.."\nЦель снаряда: "..currentTurret.defence_type.name
	...
	if damage.breachChance > 0 then
		desc = desc.."\nВероятность пробоины: "..math.floor(damage.breachChance * 10).."% (Скорректировано: "..math.floor((100 - 10 * damage.fireChance) * (damage.breachChance/10)).."%)"
	end
	if damage.stunChance > 0 then
		desc = desc.."\nВероятность оглушения: "..math.floor(damage.stunChance * 10).."% ("..math.floor((damage.iStun > 0 and damage.iStun) or 3).." сек.)"
	end
	if vunerable_weapons[currentTurret.blueprint] then
		desc = desc.."\nДлительность эффекта: "..math.floor(vunerable_weapons[currentTurret.blueprint]).." сек."
	end
	...
	for _, sysName in ipairs(systemNameList) do
		if shipManager and shipManager:HasSystem(Hyperspace.ShipSystem.NameToSystemId(sysName)) then
			...
			if system.table.tooltip_type == 1 then
				...
				Hyperspace.Mouse.tooltip = "Перевести турель в атакующий режим."
			elseif system.table.tooltip_type == 2 then
				Hyperspace.Mouse.bForceTooltip = true
				Hyperspace.Mouse.tooltip = "Перевести турель в оборонительный режим."
			...
			elseif system.table.tooltip_type == 3 or system.table.tooltip_type == 3 then
				Hyperspace.Mouse.bForceTooltip = true
				Hyperspace.Mouse.tooltip = "Используйте эту кнопку, чтобы чтобы включать и выключать автоматический режим огня.\n\nЛЕВ. CTRL + ВЫБОР ЦЕЛИ меняет режим турели на противоположный"
			end
		end
	end
end)
...
local key_names = {
	SDLK_UNKNOWN = {index = 0, name = "Неизвестно"},
	...
	SDLK_LAST = {index = 323, name = "Последняя"},
}
...
script.on_internal_event(Defines.InternalEvents.PRE_CREATE_CHOICEBOX, function(event)
	if hotkey_events[event.eventName] then
		...
		event:AddChoice(event, "Текущая клавиша: "..s, emptyReq, false)
	end
end)
	...
	if event.eventName == "COMBAT_CHECK_HOTKEYS" then
		for choice in vter(choiceBox:GetChoices()) do
			if string.sub(choice.text, 1, 2) == "OG" then
				...
				choice.text = choice.text.." (в настоящее время это клавиша \""..s.."\""
			end
		end
	end
end)
